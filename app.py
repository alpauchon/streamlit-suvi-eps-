...
# -----------------------------------------------------------------------------
# Page Questions ? (Pose de questions par les élèves et réponse de l'enseignant)
# -----------------------------------------------------------------------------
    ...
    if st.session_state["role"] == "student":
        ...
        question_text = st.text_area("Pose ta question ici :", key="question_input")

        if st.button("Envoyer la question"):
            if question_text.strip():
                questions = load_questions()
                questions.append({
                    "élève": st.session_state["user"],
                    "question": question_text,
                    "réponse": "",
                    "traitée": False
                })
                save_questions(questions)
                st.success("Ta question a été envoyée à l'enseignant !")
            else:
                st.warning("❗ Veuillez entrer une question avant d’envoyer.")

        # Affichage des réponses reçues
        st.subheader("📬 Réponses à mes questions")
        questions = load_questions()
        my_questions = [q for q in questions if q["élève"] == st.session_state["user"]]
    ...
    # Interface Enseignant
        ...
        st.subheader("📌 Répondre aux questions des élèves")
        questions = load_questions()
        unanswered_questions = [q for q in questions if not q["traitée"]]

        if unanswered_questions:
            for i, q in enumerate(unanswered_questions):
                st.write(f"👦 **{q['élève']}** a demandé : ❓ *{q['question']}*")
                response_text = st.text_area(f"Réponse à {q['élève']}", key=f"response_{i}")

                if st.button(f"Envoyer réponse à {q['élève']}", key=f"send_response_{i}"):
                    if response_text.strip():
                        q["réponse"] = response_text
                        q["traitée"] = True
                        save_questions(questions)
                        st.success(f"✅ Réponse envoyée à {q['élève']}.")
                    else:
                        st.warning("❗ Veuillez entrer une réponse avant d’envoyer.")
        else:
