...
def load_data():
    try:
        ...
        # Gestion de la colonne StudentCode
        if "StudentCode" not in df.columns:
            df["StudentCode"] = ""
        else:
            df["StudentCode"] = df["StudentCode"].fillna("")
        # Assurer que la colonne Pouvoirs est une chaîne de caractères
...
if st.session_state["role"] is None:
    with st.container():
        ...
        st.title("Connexion")
        access_mode = st.radio("Sélectionnez votre rôle", options=["Enseignant", "Élève"])
        if access_mode == "Enseignant":
            ...
            if st.session_state["students"].empty:
                ...
                student_row = st.session_state["students"].loc[
                    st.session_state["students"]["Nom"] == student_name
                ].iloc[0]
                if student_row["StudentCode"] == "":
                    ...
                    if st.button("Enregistrer et se connecter", key="student_first_conn"):
                        if new_code != new_code_confirm:
                            ...
                            idx = st.session_state["students"].index[
                                st.session_state["students"]["Nom"] == student_name
                            ][0]
...
if not st.session_state["accepted_rules"]:
    with st.container():
        ...
        | Aide-coach | 250 | Coopération + Engagement | Accompagne un élève en difficulté. |
        | Coordinateur.rice de groupe | 300 | Coopération | Premier choix des groupes. |
        | Facilitateur.rice (social) | 250 | Coopération + Engagement | Propose des ajustements pour favoriser l’intégration. |
        | Réducteur.rice des contraintes | 200 | FAVEDS + Engagement | Accès à des versions allégées des consignes. |
        | Autonome | 200 | Stratégie + Engagement | Permet de choisir son parcours ou défi. |
        ...
        | 300 | Roi / Reine de la séquence (choix du prochain thème pour 4 à 6 cours). |
        """)
...
# -----------------------------------------------------------------------------
# Définition des pages en fonction du rôle
# -----------------------------------------------------------------------------
    ...
    with st.container():
        ...
        if st.session_state["role"] == "teacher":
            ...
            if st.button("Enregistrer modifications", key="save_teacher"):
                ...
                if invalid_rows:
                    st.error(f"Erreur : les élèves suivants ont une répartition invalide : {', '.join(invalid_rows)}")
                else:
            ...
            if st.button("Enregistrer modifications", key="save_student"):
                ...
                if invalid_rows:
                    st.error("Erreur : la somme de vos compétences dépasse vos points disponibles.")
                else:
...
# -----------------------------------------------------------------------------
    ...
    with st.container():
        ...
        if st.session_state["role"] == "teacher":
            ...
            st.info(f"Connecté(e) en tant que {selected_student}.")
        ...
        if selected_student:
            ...
            with onglets[0]:
                ...
                store_items = {
                    ...
                    "Maître des groupes (1h30) ou doubler ses points de compétences": 100,
            ...
            with onglets[1]:
                ...
                if st.button("Acquérir ce rôle", key="acheter_role"):
                    ...
                    if int(student_data["Points de Compétence"]) >= role_cost and all(student_compétences[comp] > 0 for comp in required_competences):
